...
3. **Tr（trigger-insensitive regularization）作為獨立機制，未能重現論文宣稱的效果，且其
   ...
   **進一步嘗試把 Eq.10 重新詮釋為「逼進 $W_{pre}$ 未被使用的子空間」（大 k Tr），實測 k∈{64,128,256,384,512}
   結果同樣呈現與 k≤32 時相同的鋸齒狀高變異，且部分 seed 因訓練崩潰而缺失資料，**
   **目前資料不足以判定任何一個大 k 值系統性優於其他——詳見「衍生假說：大 k Tr」小節。**
...
### 衍生假說：大 k Tr（脫離全域表徵空間，而非狙擊 trigger）——實驗已跑完，結果如下
...
每個 k 三個 seed，外加一組同等 λ 強度的 L2 weight decay 對照。

**⚠️ 資料完整性問題（先於結果解讀）**：18 組（5 個 k × 3 seed + 3 個 L2 seed）裡有
**3 組訓練 crash、資料不完整**：k=128 seed1（零 epoch 資料，訓練完全沒跑出結果）、
k=256 seed1（在 epoch 3 中斷）、k=256 seed2（在 epoch 11 中斷）。這些不是「隨機缺失」——
crash 很可能跟該次 run 本身數值不穩定（如梯度爆炸）相關，而這種不穩定在 Tr 機制下更可能
發生在效果被壓得極端（更低或訓練直接失敗）的那一側。換句話說，用剩下的 seed 計算平均，
方向可能是**樂觀偏誤**（高估效果），不是低估。

**結果（final ASR，僅列可用 seed 的平均，不完整組已標注）：**

| 設定 | 有效 seed 數 | 平均 final ASR |
|---|---|---|
| L2 weight decay（同等強度對照） | 3/3 | ≈0.9960 |
| Tr, k=64 | 3/3 | ≈0.8940 |
| Tr, k=128 | **2/3**（seed1 零資料） | ≈0.5908（不完整，見下方解讀） |
| Tr, k=256 | **1/3**（seed1 停於 epoch3，seed2 停於 epoch11） | 僅 1 筆有效值 0.7140，**無法計算均值** |
| Tr, k=384 | 3/3 | ≈0.9655 |
| Tr, k=512 | 3/3 | ≈0.8698 |
...
**結論二（誠實表述，不誇大）：k=128、k=256 目前不能被判定為「最佳 k」。**
k=128 的 0.59 確實是表中最低數字，但只建立在 2 個 seed 上，且缺失的第 3 個 seed 是完全
crash、不是普通的高值——考慮到 crash 可能與極端效果（樂觀方向）相關，這個 0.59 很可能是
被高估的。k=256 更嚴重：3 個 seed 裡 2 個中途崩潰，只有 1 個完整值（0.714），**n=1 不是
「不確定的均值」，是根本沒有均值可言**，不能拿來跟其他 k 比較。
把現有的點連起來看（64→0.89、128→0.59（不完整）、256→0.71（n=1）、384→0.97、512→0.87），
震盪幅度跟 k≤32 時看到的鋸齒狀 pattern是同一種形狀——不是一個乾淨的 U 型或單調趨勢。
這跟本專案之前 k=2「最優值」的故事是同一種陷阱：當時 k=2 看起來最好，也是因為 seed 數不足，
補完多 seed 後那個最優點直接消失；現在 k=128/256 的情況比當時的 k=2 還更脆弱（k=2 至少
3 個 seed 都跑完了，現在 k=128 只有 2 個、k=256 只有 1 個）。

**目前唯一站得住腳的結論是**：大 k Tr 確實比 L2 weight decay 有更強的效果（排除偽裝 L2
假說），但在 k=64/128/256/384/512 這個範圍內，究竟哪個 k 最好、或是否存在真正的最優 k，
**現有資料因缺失 seed 而無法回答**。在補完 k=128 的 seed1、k=256 的 seed1/seed2 之前，
不應在正式結論中寫「k=128 或 256 是最佳」，只能寫「初步、不完整的資料顯示 k=128 附近
可能較優，但因 3/18 run crash，此觀察待驗證」。
...
共用），不代表它是「已驗證的穩定基準」。詳細解讀見「Tr 機制總結」章節。
...
- 機制2 SVD 截斷 k（論文未明講是否截斷、截斷到多少）——已消融 k∈{1,...,8,32,64,128,256,384,512,1024}，
  ...
  大 k（64–512）延伸驗證同樣呈現高變異且部分 seed 缺失，**k=128/256 目前的偏低均值可能是
  crash 造成的資料不完整偏誤，不能視為「找到最優 k」**；且已用獨立的 SVD alignment 實驗
  （含 3-way own/foreign/control 細分）排除「存在固定 trigger 子空間、只是 k 沒找對」這個
  解釋（見「機制2延伸」章節）
...
- 論文數字多為多次 run 取最好；我們除 LoRA+Cl（BadNet, 11 seed）、LoRA+Tr k=2（4 seed）、
  LoRA+Tr k=8（6 seed）、大 k Tr（k=64/384/512 各 3 seed，k=128/256 因 crash 不完整）外，
  其餘均為單次 seed=0——鑑於多組多 seed 結果都顯示極高變異性，其餘單次數字的代表性應保守看待
...
### 有明確查證路徑（若有餘裕可執行）
- **補完大 k Tr 缺失的 seed：k=128 seed1（零資料重跑）、k=256 seed1（epoch3 中斷重跑）、
  k=256 seed2（epoch11 中斷重跑）——在補完之前，「k=128/256 可能最優」只能算未驗證觀察，
  不能寫進正式結論。建議優先查一下這幾次 crash 的原因（例如是否是較大 k 的截斷矩陣尺寸
  導致 OOM），避免重跑後又用一樣的設定再 crash 一次。**
...
- 對 λ 網格、k=32/1024 等其餘設定重複多 seed 驗證，確認雜訊程度是否一致；也尚未測過縮小 λ 的方向
- SVD alignment 實驗中「中毒模型差距比 clean 模型大」的觀察（2-way 版本）——單次結果，
  未經多 seed 驗證，若有餘裕可延伸確認是否為真訊號
