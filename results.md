...
**Cl+Tr+Pt 四格使用的 layer 選取為 `[-6:]`（我們對論文「top three layers」的猜測，
即最後 3 個完整 transformer layer）。**
...
Pt 本身也單獨有效（LoRA InSent baseline 75.36%→Pt alone 21.01%）——這代表機制3不完全依賴
機制1、2先鋪路（見「機制3在 DoRA 上的理論分析」推導3的但書）。

**觀察 2：** DoRA 側呈現不同模式——Cl+Tr+Pt（7.70%）比 Cl+Pt（7.37%）還差一點，
Tr 在 DoRA 上對完整疊加是輕微拖累，不是中性也不是助力。Tr 對 LoRA/DoRA 的貢獻方向不一致，
記錄但不深究成因。

**觀察 3：** DoRA+Cl+Tr+Pt 在 InSent 上，我們的 ASR（7.70）依然低於論文對應數字（21.56），
是目前所有對照格子裡唯一一處在「論文有直接測過的精確對照組」上表現優於論文的案例，
且相當穩固。原因未探究，可能候選：論文該格也是單次或少次結果、我們的 k=8/6-module Pt
選擇剛好在此設定下更有效、或純粹雜訊。不做進一步因果推論。
...
3. **Tr（trigger-insensitive regularization）作為獨立機制，未能重現論文宣稱的效果，且其
   ...
   兩組獨立訓練，ASR 範圍分別為 57–100% 與 67–100%；已查證 Eq.9→10→11
   ...
   **把 Eq.10 重新詮釋為「逼進 $W_{pre}$ 未被使用的子空間」（大 k Tr），實測 k∈{64,128,256,384,512}
   ...
   有專屬子空間、只是超參數沒調對」（k 已測滿全範圍，含大 k 重新詮釋版本，仍無穩定效果），
   另一端是這個前提本身（SVD alignment 實驗直接顯示不存在可分離的 trigger 子空間）——兩端
   互相印證，指向更根本的問題。跨層 penalty 平均化 vs 加總這個實作選擇是否也是成因之一，
   目前證據不足以判定（見「λ 網格驗證」），是這條推理鏈上尚未真正關閉的一段。
...
4. Eq.9→10→11 方向與符號已逐字核對論文原文，確認實作邏輯正確——排除「做反方向」的可能
5. 14 個矩陣（跨 layer 0/3/6/9/12/15/18/21/23 的 query/value）SVD 檢查：全數滿秩、
...
6. **獨立的 SVD alignment 實驗（真實 forward hidden state，真實 IMDB label=1 句子，
...
上述證據 4、5 是實作核對，不是「排除的可能性」，僅確認懲罰方向與符號正確、且理論上的滿秩退化
...
4. **這條鏈尚未關閉的一段：** Eq.10 跨層 penalty 該平均化還是加總，論文未明講，我們現行
   設定是平均化。曾試著測試「改成加總（對齊論文 λ 網格）會不會更好」，但這個比較目前只有
   單次結果，而我們已經從 k=2 的四次獨立訓練知道，同一設定（k=2, λ=10, 平均化）單次結果
   可以在 0.5732–0.9967 之間亂跳——比較基準本身的變異幅度遠大於待比較的幾個 λ 值彼此的
   差距，證據力不足以判斷平均化或加總哪個更好（詳見「λ 網格驗證」）。這條路徑事實上
   **尚未真正排除**，需要每個 λ 值補上多次獨立訓練才能重新檢驗。
...
「找錯 k」的問題。** (e) 跨層 penalty 該平均化還是加總這個實作選擇本身是否是震盪成因之一，
目前證據不足以排除或確認（見上方「尚未關閉的一段」）。五者皆為推測或部分證實，非完全
確認的因果結論。
...
### 機制3 scaling s 數值（6-module，3 個完整 transformer layer）
...
多出的 layer21 兩個 module 的 $s$ 值與 layer22/23 同量級，支持「top three layers 為最後
三個完整 transformer layer」這個猜測是合理的。
...
### λ 網格驗證（固定 k=2, LoRA, BadNet）—— 證據不足，無法判定
...
**結論：證據不足，無法判定「平均化 vs 加總」哪個更好。** 表中拿來當「主線設定」對照基準的
那一格（Test CA 0.9561、ASR 0.5732），其實就是「k 消融」章節 k=2 四次獨立訓練裡的第 1 次；
而那四次的完整範圍是 0.5732–0.9967，完全涵蓋了三個「加總」λ 值測到的 ASR（0.9461/0.7404/
0.8306）。也就是說，這個比較本質上是「單次結果 vs 單次結果」，而同一設定（k=2, λ=10,
平均化）單次結果本身的變異幅度（0.4 以上）遠大於這幾個 λ 值彼此的差距——不能排除「平均化
看起來比較好」單純是抽樣運氣（抽到 k=2 四次裡最低的那一次），不是加總/平均本身的真實差異。
三個加總值彼此非單調（λ=48→240 降、240→480 又升）同樣可能只是雜訊，不是真實的 λ 效應。
**這個實驗需要每個 λ 值都補上多次獨立訓練才能重新判定方向，現有的單次比較不構成證據**——
也因此，加大或縮小 λ 的方向都尚未真正測試過，不應宣稱任何一個方向已被排除。
...
- 機制2 SVD 截斷 k（論文未明講是否截斷、截斷到多少）——已消融 k∈{1,...,8,32,64,128,256,384,512,1024}，
  ...
  這個解釋（見「機制2延伸」章節）
- 機制2跨層 penalty 加總 vs 平均（論文 Eq.11 未明講）——已用單次結果測試對齊論文網格的
  加總形式，但比較基準（平均化主線設定）本身也只有單次結果，且同設定下已知變異可達
  0.4 以上（見「λ 網格驗證」），無法判定加總或平均哪個更好，也不確定論文原意是哪一種；
  加大、縮小 λ 的方向都尚未真正測過
- **訓練腳本未鎖定 cuDNN 決定性**（見「方法論附註」）——所有既有多次獨立驗證結果的每一次
  都應理解為獨立隨機嘗試，不是可重現身份；日後若需要可重現的單次結果，需補上
  `cudnn.deterministic=True` 等設定，尚未套用
- 機制3「top three layers」——我們猜測為最後 3 個完整 transformer layer（`[-6:]`，
  6 個 module），總覽表已採用此設定
...
- **λ 網格的每個 λ 值都補上多次獨立訓練**，才能重新判定平均化 vs 加總哪個更好——現有
  比較全部只有單次結果，證據力不足（見「λ 網格驗證」）；也對 k=32/1024 等其餘設定重複
  多次獨立訓練，確認雜訊程度是否一致；加大、縮小 λ 的方向都尚未真正測過
...
- 機制3「top three layers」指哪三層：論文僅給模糊描述，我們的 6-module 設定是合理延伸，
  非論文確認答案
...
- **Tr 機制本身（含大 k 版本）看不到穩定成功的根本原因：已沿著「trigger 子空間是否存在→
  ...
  lr 選錯的可能性；平均化 vs 加總這個實作選擇是否是成因之一則仍未排除（證據不足，見上）——
  仍無法定論是論文本身效果不穩定、逐層 k 不一致、還是正則化訊號天生微弱**（皆為推測，
  非確認結論）
