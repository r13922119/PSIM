...
**總覽對照表採用 k=8。k=2/3 曾一度被判定為最佳值，但四 seed 完整驗證後已推翻此結論
（ASR 範圍 0.5732–0.9967，標準差達均值的四分之一），k=8 是唯一未經雜訊污染的基準，見「機制2消融」。**
...
3. **Tr（trigger-insensitive regularization）單獨效果對超參數極度敏感，呈鋸齒狀非單調震盪，
   且完整 Pt 消融顯示 Tr 對最終疊加結果的貢獻遠小於先前假設。**
   k 掃描（k=1–7 全測）顯示無平滑最優值；四 seed 驗證進一步證實 k=2/3 的單次低點是雜訊
   （ASR 0.5732–0.9967，標準差達均值 1/4），不存在穩定的「最佳 k」。λ 網格驗證（對齊論文
   {1,5,10,15,20}）同樣顯示非單調震盪，排除「平均化強度不足」假說。InSent 上 LoRA 的
   Cl+Pt≈Cl+Tr+Pt（6.49% vs 6.71%）進一步證實 Tr 在完整疊加裡貢獻有限，主要改善來自 Cl。
   （此結論歷經四次修正：單調遞減→平滑甜蜜點→鋸齒震盪→現確認為近乎隨機、且對最終結果
   貢獻有限，見「機制2消融」）
...
**結論：k=2「最佳值」正式推翻。** 四個 seed 的 ASR 落在 0.5732–0.9967 之間，標準差達均值的
四分之一——這不是「效果不錯但不穩定」，是「k=2 本身跟其他 k 值一樣，是雜訊等級的隨機震盪，
seed=0 那次 57.32% 只是運氣好抽到的低點」。k=8（單次 93.73%）雖然數字較差，但至少不是靠
單一 seed 的巧合撐起來的宣稱——總覽表主線維持使用 k=8，是唯一目前有依據的選擇（其他 k 值
皆未做多 seed 驗證，不能排除同樣的雜訊問題，但 k=8 是最初、也是被最多其他消融實驗共同使用
的基準，變更基準本身需要更強證據）。
...
不是論文明確指定的答案。**加上此輪 k 值多 seed 驗證顯示任何 k 的效果都近乎雜訊，這進一步
暗示 Eq.10 的軟化近似（不論如何截斷）可能本身就是一個效果不穩定的正則化項，不只是截斷維度
選擇的問題。**

### 論文對照

論文 Table 5（RoBERTa+BadNet）Tr alone: CA 95.33 / ASR 13.42。我們的 Tr alone 結果在
k=8（單次 93.73%）與 k=2（四 seed 均值 78.03%±21.09）之間震盪，均遠高於論文數字，
...
- 機制2 SVD 截斷 k（論文未明講是否截斷、截斷到多少）——已消融 k∈{1,...,8,32,1024}，
  且對 k=2 做過四 seed 驗證，確認任一單一 k 的表現都無法穩定重現，效果近乎雜訊
...
- 論文數字多為多次 run 取最好；我們除 LoRA+Cl（BadNet, 11 seed）與 LoRA+Tr k=2（BadNet,
  4 seed）外，其餘均為單次 seed=0——鑑於這兩組多 seed 結果都顯示極高變異性，其餘單次數字
  的代表性應保守看待
...
### 有明確查證路徑（若有餘裕可執行）
- 探究 Tr 對 k 的震盪成因：計算 W_pre 前幾個右奇異向量 v_i 與 trigger embedding 的 cosine
  similarity，檢驗是否有特定方向與 trigger 對齊（不需重新訓練，約 10 分鐘）——
  鑑於震盪已被證實近乎雜訊，此查證的預期價值降低，優先度已下調
- 對其他 k 值（8、32 等）與 λ 網格重複多 seed 驗證，確認雜訊程度是否一致
...
- 機制1 對 DoRA 的結構性限制（mag_scale 要求整體一致性）：已確認無法在不犧牲 RoRA
  核心設計的前提下修復，不再列為待辦
- Tr 機制本身對超參數（k、λ）的高變異性：已用多組多 seed 實驗證實，非實作錯誤導致，
  但根本原因（是否為訓練動態固有的不穩定性，或 Eq.10 這個正則化項本身設計上的問題）未定論

### 其他
- threshold-based SVD 截斷（用奇異值門檻而非固定 k）—— 只是想法，沒有實作；
  且已知奇異值譜平滑無斷崖（無明顯自然截斷點），且鑑於 k 本身已證實效果近乎雜訊，
  預期收益更低，不再列為優先
