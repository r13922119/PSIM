...
3. **Tr（trigger-insensitive regularization）單獨效果對 k 高度敏感，呈鋸齒狀非單調震盪**——
   完整掃描 k=1–7 後，k=2、3 並列最佳（ASR 57.32%），但 k=4（82.62%）、k=5（97.58%）、
   k=6（78.77%）、k=7（87.13%）之間劇烈跳動，不是以 k=2 為中心的平滑曲線。這代表 Tr 機制
   對 k 的敏感度可能近乎不穩定/不可預測，而非存在一個乾淨的「中間最優值」（此結論已兩度修正：
   先由稀疏取樣誤判為單調遞減，現由完整掃描顯示連「單一甜蜜點」的描述都不準確，
   見「機制2消融」章節）。
...
被加入的新奇異方向剛好與 trigger 方向的夾角變化劇烈（見奇異值精細分析，Section 待補）；
