...
‡ 單次 seed=0 結果；k=8 的 6-seed 均值為 89.8%±11.5%（範圍 67.3–99.7%）——單次數字同樣不代表典型表現，見「Tr 機制總結」章節。
...
3. **Tr（trigger-insensitive regularization）作為獨立機制，未能重現論文宣稱的效果，且其
   ...
   兩組多 seed 驗證，ASR 範圍分別為 57–100% 與 67–100%，標準差達均值的 13–24%；λ 網格驗證
   ...
   （見「機制2延伸：trigger 子空間假設的直接驗證」）——這排除了 Tr 失敗的一個常見候選解釋
   （即「只是還沒找到對的 k」），因為若根本沒有可分離的 trigger 子空間，那麼任何固定 k 的
   Eq.10 都不可能穩定生效，此發現與 k 消融的高變異結果互相印證。** 綜合以上，Tr 單獨使用時
   「看不到穩定成功」——不是零效果（InSent 上與 Cl 疊加時近乎中性，BadNet 上小幅正貢獻），
   但從未穩定重現論文的強力效果，此結論已排除超參數選擇、方向錯誤、trigger 子空間存在但
   k 選錯三類常見解釋，指向更根本的問題（見「Tr 機制總結」的候選解釋）。
...
2. k=2 四 seed 驗證：均值 78.0%±21.1%（範圍 57–100%）
3. k=8 六 seed 驗證：均值 89.8%±11.5%（範圍 67–100%）
...
**已排除的解釋：** 懲罰方向錯誤；λ 平均化強度不足；k 選擇不佳（已測滿全範圍）；
單一矩陣的特例（已擴大驗證至 14 個矩陣）；**存在固定 trigger 子空間、只是 k 還沒找對
（已用獨立的 SVD alignment 實驗直接排除，見下方章節）**。
...
（現已有直接證據支持這個候選，見下方章節），這代表 Eq.10 的整個設計前提——「trigger 有
...
### 結果（Global Rank Distribution，pool across layer∈{0,3,6,9,12,15,18,21,23}×{query,value}）
...
（`real_trigger` = `mn`、InSent 句；`control` = `abysmal`、`perfect`、`the`、`movie`、
`watched`、`3D movie`、"This movie is abysmal/perfect"、decoy 長句；矩陣皆為 1024×1024，
故 top-k 的分母上限是 1024。）

### 結論

**不存在可分離的 trigger 專屬子空間。** real_trigger 在三個模型的 top-8 集中度
（39.44% / 42.22% / 48.33%）皆**低於**（不是高於）control 詞（51.60% / 49.01% / 50.62%）——
跟「trigger 應該更集中在某個特定子空間」的預期方向完全相反。且這個「real_trigger 比
control 更分散」的現象，**在從未中毒過的 clean 模型裡也存在**（48.33% vs 50.62%），
證明這不是 backdoor 訓練造成的，而是這批特定字詞本身 hidden state 分佈的內在差異，
跟是否為 trigger 無關。

**附帶發現：** 不論 real_trigger 或 control，訊號幾乎全數（top-256 達 99.3–100%，
top-128 已達 95–98%）落在 1024 維裡的前 256 維——1024 維裡有 700 維以上，對任何自然語言
輸入（trigger 或非 trigger）幾乎都沒有影響力。這是所有測過的字詞共享的性質，不是
trigger 專屬的。

**未經統計驗證、保守記錄的觀察：** 中毒模型的 real_trigger/control 差距
（badnet 12.2pp、insent 6.8pp）比 clean 模型（2.3pp）更大。這可能暗示 poisoning 訓練
讓 control 詞的集中度相對變高了一些，但這只是單次跑的結果（n=180 vs n=810 不對稱），
沒有做多 seed 驗證。鑑於「機制2消融」章節已多次證明看起來像訊號的東西一驗證就消失
（k=2 的故事），**此觀察不作為結論，僅記錄供未來若有時間可延伸驗證**。

### 衍生假說：大 k Tr（脫離全域表徵空間，而非狙擊 trigger）——實驗已送出，結果未回

既然沒有 trigger 專屬子空間，但兩組（trigger 跟 control）的訊號都幾乎全部集中在
前 256 維以內，那 Tr 的設計意圖可以重新詮釋為：不是「精準狙擊 trigger 方向」（已證明
不存在這種可分離方向），而是「把 ΔW 逼進 $W_{pre}$ 幾乎沒在用的那 700+ 維空間」——
這是定性不同、且更站得住腳的機制解讀，**但目前僅為推論，尚無實驗結果支持或反駁**。

測試計畫：k ∈ {128, 256, 384, 512}，Tr-alone，BadNet，3 seed（時間有限，非完整規格），
外加一組同等 λ 強度的 L2 weight decay 對照（**必須做這組對照**，因為已證明 k→1024 時
Eq.10 會退化成 L2，k 開太大測出的效果如果只是重新發現「L2 有點用」，跟這個假說想驗證的
「脫離表徵空間」是兩件不同的事）。結果待補。
...
| 2 | 0.9605 | 0.9725 |
| 3 | 0.9572 | 0.5787 |
| **均值 ± 標準差** | 0.9583 ± 0.0017 | **0.7803 ± 0.2109** |
...
| 4 | epoch 6 | 0.9483 |
| 5 | epoch 7 | 0.8361 |
| **均值 ± 標準差** | — | **0.8979 ± 0.1146** |

**結論：k=2、k=8「最佳值」皆已推翻，兩者皆為雜訊等級的高變異。** k=2 四 seed 範圍
0.5732–0.9967（標準差達均值 27%），k=8 六 seed 範圍 0.6733–0.9967（標準差達均值 13%）。
...
（k=8 六 seed 均值 89.8%±11.5%；k=2 四 seed 均值 78.0%±21.1%）均遠高於論文數字，
...
- 機制2 SVD 截斷 k（論文未明講是否截斷、截斷到多少）——已消融 k∈{1,...,8,32,1024}，
  ...
  且已用獨立的 SVD alignment 實驗排除「存在固定 trigger 子空間、只是 k 沒找對」這個解釋
  （見「機制2延伸」章節）
...
- 論文數字多為多次 run 取最好；我們除 LoRA+Cl（BadNet, 11 seed）、LoRA+Tr k=2（4 seed）、
  LoRA+Tr k=8（6 seed）外，其餘均為單次 seed=0——鑑於三組多 seed 結果都顯示極高變異性，
  其餘單次數字的代表性應保守看待
...
### 有明確查證路徑（若有餘裕可執行）
- **大 k Tr 假說（k=128/256/384/512，含 L2 weight decay 對照，見「機制2延伸」章節）：
  實驗已送出，結果尚未回收。**
- 驗證「同一 k 在不同層代表不同截斷比例」是否為震盪真正成因：可對每層各自用不同的 k
  （依該層 σ1/σ2 比值調整），而非全域統一 k，觀察震盪是否減緩——工程量較大，未執行
- 對 λ 網格、k=32/1024 等其餘設定重複多 seed 驗證，確認雜訊程度是否一致
- SVD alignment 實驗中「中毒模型差距比 clean 模型大」的觀察（見「機制2延伸」）——
  單次結果，未經多 seed 驗證，若有餘裕可延伸確認是否為真訊號
...
- 機制3「top three layers」指哪三層：論文僅給「primarily govern the classification
  decision boundary」一句，無其他線索。我們的 6-module 修正是合理延伸，非論文確認答案
- σ_max 近似 A_1 在 DoRA 封頂結構下的誤差界：需獨立的理論推導，非查資料可解
- 機制1 對 DoRA 的結構性限制（mag_scale 要求整體一致性）：已確認無法在不犧牲 RoRA
  核心設計的前提下修復，不再列為待辦
- **Tr 機制本身看不到穩定成功的根本原因：已窮盡排除方向錯誤、超參數選擇、單一矩陣特例、
  trigger 子空間存在但 k 選錯四類解釋，仍無法定論是論文本身效果不穩定、逐層 k 不一致、
  或正則化訊號天生微弱**（候選見「Tr 機制總結」，皆為推測，非確認結論）

### 其他
- threshold-based SVD 截斷（用奇異值門檻而非固定 k）—— 只是想法，沒有實作；
  且已知奇異值譜平滑無斷崖（14 矩陣驗證），且鑑於 k 本身已證實效果近乎雜訊，
  預期收益更低，不再列為優先
