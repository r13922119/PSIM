...
‡ 單次 seed=0 結果；k=8 的 6-seed 均值為 89.8%（範圍 67.3–99.7%）——單次數字同樣不代表典型表現，見「Tr 機制總結」章節。
...
2. **Cl（clean-strengthened regularization）單獨是三個機制裡效果最強、也最穩定的**——兩個
   ...
   一致。11-seed 驗證顯示雖有變異（13.5–49.5%），但均值仍明顯優於 baseline，方向穩固。
3. **Tr（trigger-insensitive regularization）作為獨立機制，未能重現論文宣稱的效果，且其
   效果本身呈現接近隨機的高變異。** 完整 k=1–7 掃描無平滑趨勢；k=2（4 seed）與 k=8（6 seed）
   兩組多 seed 驗證，ASR 範圍分別為 57–100% 與 67–100%；λ 網格驗證
   ...
   （即「只是還沒找到對的 k」）。**
   **進一步嘗試把 Eq.10 重新詮釋為「逼進 $W_{pre}$ 未被使用的子空間」（大 k Tr），實測 k∈{64,128,256,384,512}
   結果同樣呈現與 k≤32 時相同的鋸齒狀高變異——補完更多獨立跑完的結果後，原本看起來偏低的
   k=128、k=256 均值都明顯往其他 k 的水準回歸（見「衍生假說：大 k Tr」），**
   **目前資料不足以判定任何一個大 k 值系統性優於其他。**
...
2. k=2 四 seed 驗證：範圍 0.5732–0.9967
3. k=8 六 seed 驗證：範圍 0.6733–0.9967
...
**與 Cl、Pt 的對比：** Cl（11-seed）與此處 Tr（k=2/k=8 共 10 個 seed）都呈現高變異，
...
1. **v1（初版）**：用 trigger 的原始 embedding（未經任何 transformer layer）對每一層
   ...
   （皆 ≈0.025），且此現象在 clean 模型中同樣存在；但一開始誤判「the」也呈現跟其他詞
   一樣的 rank-0 凸起，經覆核後發現「the」實際上在 54 個 layer×module 組合中從未出現
   rank-0 凸起，而 `mn`/InSent/其他實詞都會——這個錯誤已在對話中糾正，且因為 v1 本身
   方法論有缺陷，此版本結果不作為最終結論依據，僅記錄修正過程。
...
**動機修正**：2-way 版本把 `mn`（BadNet trigger）跟 InSent 句都算進同一個 `real_trigger`，
但對 `poisoned_roberta_large_badnet` 而言 InSent 是一個它從未學過的「外來」trigger，反之亦然。
混在一起會稀釋「模型自己真正學到的攻擊」跟「一個不相干的 trigger 形狀的句子」之間的對比。
細分後才是同一模型內部更嚴謹的比較。
...
**⚠️ 資料完整性提醒**：目前只確認記錄了 top-8 這一欄的 3-way 數字（來自對話中使用者直接
貼出的實測輸出）；top-32/64/128/256 以及各組精確 n（原 180 筆如何拆分成 own/foreign 各半）
尚未在文件中留存，需要重新貼上完整 log 才能補完——**在補完之前，下方結論僅基於 top-8 這一個
資料點，不代表已驗證跨所有 k 尺度的完整模式**。
...
其表徵依然沒有集中到任何可辨識的低維子空間，反而比一個隨機 control 詞更分散。這進一步
排除了「2-way 的訊號只是因為 InSent 拉低了平均」這個可能的反駁。
...
讓 control 詞的集中度相對變高了一些，但這只是單次跑的結果（n=180 vs n=810 不對稱），
沒有做多 seed 驗證。**此觀察不作為結論，僅記錄供未來若有時間可延伸驗證。**
...
外加一組同等 λ 強度的 L2 weight decay 對照。

**⚠️ 關於「seed」的重要方法論修正：** 訓練 code（`variant_finetune.py`）確實對
`torch`/`numpy`/`random` 都設了種子，但**沒有設定 `torch.backends.cudnn.deterministic`**，
這代表即使 seed 數值相同，GPU 上 cuDNN 的非決定性演算法仍會讓兩次執行從第一個 backward
step 就開始出現浮點差異，訓練 20 epoch 後可能放大成明顯不同的最終 ASR。已直接證實：
補跑「k=128 seed=1/2」與「k=256 seed=1」時，得到的數字與舊記錄的對應 seed 編號並不一致。
**這代表「seed=N」在本專案的訓練流程裡不是一個可重現、可追蹤身份的實驗單位，只是啟動
一次獨立隨機訓練的引子。** 因此，以下不再按 seed 編號逐一列出或比對，只計「每個 k 底下
累積了幾筆獨立跑完的結果」，並直接合併新舊資料計算均值。

（另外，先前記錄的三組 crash 中，k=256 seed=2 的一次重跑已確認是單純的 GPU 記憶體不足
——同時有另一個行程占用了該卡 42GB／47GB 的記憶體，屬於共享 GPU 資源排擠，與 Tr 訓練
本身的數值穩定性無關；此組目前仍無有效資料。）

**目前累積結果（合併新舊資料，final ASR，僅計已完整跑完 20 epoch 的結果）：**

| 設定 | 已完整跑完次數 | 合併均值 |
|---|---|---|
| L2 weight decay（同等強度對照） | 3 | ≈0.9960 |
| Tr, k=64 | 3 | ≈0.8940 |
| Tr, k=128 | **4**（舊 2 筆+新 2 筆：0.8185、0.8724） | ≈0.7181 |
| Tr, k=256 | **2**（舊 1 筆 0.7140+新 1 筆 0.9835） | ≈0.8488 |
| Tr, k=384 | 3 | ≈0.9655 |
| Tr, k=512 | 3 | ≈0.8698 |

**結論一（已確認）：大 k Tr 不是「偽裝的 L2」。** L2 對照組本身幾乎沒有效果（≈99.6%，
接近 baseline），而所有測過的大 k Tr 條件都顯示出比 L2 更低的 ASR。這排除了「大 k 效果
只是重新發現 L2 weight decay 有點用」這個混淆假說——之前證明過未截斷時 Eq.10 退化成 L2，
但這裡的效果不能單純用退化成 L2 解釋。

**結論二（更新後，更誠實）：補完更多獨立跑完的結果後，k=128、k=256 原本看起來偏低的均值
明顯往其他 k 的水準回歸。** k=128 從原本 2 筆的 0.59 上修到 4 筆的 0.72；k=256 從原本
n=1 的 0.71（不能算均值）上修到 n=2 的 0.85。五個 k 排序後（64→0.89、128→0.72、256→0.85、
384→0.97、512→0.87），k=128 仍是最低，但已經跟 k=256/512（0.85/0.87）差距很小，不再是
明顯突出的低點。這正是本專案先前 k=2「最優值」故事的重演：樣本量小的時候看起來特別好的點，
補完更多獨立結果後往均值回歸。

**目前唯一站得住腳的結論**：大 k Tr 確實比 L2 weight decay 有更強的效果（排除偽裝 L2
假說），但在 k=64/128/256/384/512 這個範圍內，究竟哪個 k 最好、或是否存在真正的最優 k，
現有資料量仍然太小、變異仍然很大，**無法給出肯定答案**，且由於 cuDNN 非決定性，未來即使
再累積更多次獨立訓練，也應該預期均值會持續小幅波動，而不是收斂到一個乾淨的數字。
...
放大成明顯不同的最終 ASR。已直接觀察到此現象（見「衍生假說：大 k Tr」補測 k=128/k=256 時，
同一 seed 編號兩次跑出的數字不一致）。

**影響範圍：** 這代表本文件所有「N-seed 驗證」章節（機制1的 11-seed、機制2的 k=2 四 seed／
k=8 六 seed），其中的「seed」都應理解為「N 次獨立隨機訓練」，不是「N 個可重現、可個別追蹤
核對的實驗單位」——重跑同一個 seed 編號不保證重現同一個結果。這不影響這些章節已得出的
「效果高變異」這個結論本身（變異本來就存在，來源多一項不會讓它變小），但精確地說，這些
變異裡混有 cuDNN 非決定性貢獻的額外雜訊，不是純粹只來自「不同初始化/不同資料順序」。
...
### k 消融（固定 lr=2e-4, LoRA, BadNet）—— 完整版，含精細掃描與多 seed 驗證
...
**k=2 的四 seed 完整驗證（BadNet, LoRA+Tr, k=2, λ=10）：**
...
**k=8 的六 seed 驗證（BadNet, LoRA+Tr, k=8, λ=10）：**
...
**結論：k=2、k=8「最佳值」皆已推翻，兩者皆為雜訊等級的高變異。** k=2 四 seed 範圍
0.5732–0.9967，k=8 六 seed 範圍 0.6733–0.9967。
兩組的均值本身也相近（78.0% vs 89.8%），且區間有大幅重疊——這代表沒有證據顯示任一測過
的 k 值系統性優於另一個，總覽表沿用 k=8 純粹是歷史慣性（最早使用、被最多其他消融實驗
共用），不代表它是「已驗證的穩定基準」。詳細解讀見「Tr 機制總結」章節。**另外，這裡的
「seed」同樣受「訓練本身的隨機性未完全鎖定（cuDNN 非決定性）」這個發現影響（見方法論附註）
——不應把 seed 0–5 當成可重現、可個別追蹤核對的六個點，只當成六次獨立訓練。**

DoRA 的 k 對照（僅測過 k=8, 32，未做多 seed 驗證，同樣可能受此雜訊問題影響）：
...
本身建立在 k=2, seed=0 這個現已知為雜訊低點的設定上，λ 網格本身是否同樣受雜訊影響未再驗證。
平均化本身不是造成落差的原因，此點仍然成立（因為對照組本身用的也是同一個 k=2, seed=0）。
**只測過對齊論文網格的加大方向（λ 加總 48/240/480 對齊 1/5/10），未測過縮小 λ 的方向，
這條路徑尚未窮盡，不應宣稱「λ 已排除」的範圍包含縮小方向。**
...
（k=8 六 seed 均值 89.8% 與範圍 67.3–99.7%；k=2 四 seed 均值 78.0% 與範圍 57.32%–99.67%）均遠高於論文數字，
且經多 seed 驗證後，此落差無法用「湊到一個更好的 k」來系統性縮小——落差的性質更接近
「Tr 這個機制本身的實作，在我們的訓練設定下就是效果不穩定」，而非「差一個超參數」。
...
- 機制2 SVD 截斷 k（論文未明講是否截斷、截斷到多少）——已消融 k∈{1,...,8,32,64,128,256,384,512,1024}，
  且對 k=2、k=8 皆做過多 seed 驗證，確認任一單一 k 的表現都無法穩定重現，效果近乎雜訊；
  大 k（64–512）延伸驗證同樣呈現高變異，補完更多獨立結果後 **k=128/256 的均值已明顯往其他
  k 回歸，不能視為「找到最優 k」**；且已用獨立的 SVD alignment 實驗（含 3-way
  own/foreign/control 細分）排除「存在固定 trigger 子空間、只是 k 沒找對」這個解釋
  （見「機制2延伸」章節）
- 機制2跨層 penalty 加總 vs 平均（論文 Eq.11 未明講）——已測試對齊論文網格的加總形式，
  結果皆劣於現行平均化設定，假說已推翻，但仍不確定論文原意是哪一種；且只測過加大方向，
  縮小 λ 的方向未測過
- **訓練腳本未鎖定 cuDNN 決定性**（見「方法論附註」）——所有既有多 seed 結果的「seed」都應
  理解為獨立隨機嘗試而非可重現身份；日後若需要可重現的單次結果，需補上
...
- 論文數字多為多次 run 取最好；我們除 LoRA+Cl（BadNet, 11 次獨立訓練）、LoRA+Tr k=2（4 次）、
  LoRA+Tr k=8（6 次）、大 k Tr（k=64/384/512 各 3 次，k=128 累積 4 次，k=256 累積 2 次）外，
...
- 對 λ 網格、k=32/1024 等其餘設定重複多次獨立訓練，確認雜訊程度是否一致；也尚未測過縮小 λ 的方向
- k=256 仍只有 2 次獨立跑完的結果（0.7140、0.9835，差距極大），k=128 已有 4 次但變異仍高
  ——若有餘裕，可再補幾次獨立訓練縮小均值的不確定範圍，但不需要再糾結「哪個 seed 編號」
