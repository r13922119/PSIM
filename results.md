...
‡ 單次 seed=0 結果；k=8 的 6-seed 均值為 89.8%±11.5%（範圍 67.3–99.7%）——單次數字同樣不代表典型表現，見「Tr 機制總結」。
...
3. **Tr（trigger-insensitive regularization）作為獨立機制，未能重現論文宣稱的效果，且其
   ...
   且已用 14 個矩陣的 SVD 檢查確認奇異值譜全數平滑滿秩（詳見「Tr 機制總結」）。綜合以上，
   Tr 單獨使用時「看不到穩定成功」——不是零效果（InSent 上與 Cl 疊加時近乎中性，BadNet 上
   小幅正貢獻），但從未穩定重現論文的強力效果，此結論已排除超參數選擇、方向錯誤兩類常見
   解釋，指向更根本的問題（見「Tr 機制總結」的候選解釋）。
...
- **Tr 機制本身看不到穩定成功的根本原因：已窮盡排除方向錯誤、超參數選擇、單一矩陣特例
  三類解釋，仍無法定論是論文本身效果不穩定、逐層 k 不一致、或正則化訊號天生微弱**
  （三個候選見「Tr 機制總結」，皆為推測，非確認結論）
