...
## 總覽對照表：我們的結果 vs 論文 Table 2/4/5（RoBERTa-only）
...
| LoRA +Cl+Tr | 95.55 / 15.62 | （論文 Table 4 無此列，只有 Cl-only/Tr-only/Cl+Tr+Pt） | 95.55 / 17.38 | （同左） |
| LoRA +Cl+Tr+Pt | 95.83 / 13.20 | 95.99 / 6.49 | 95.83 / 10.67 | 95.83 / 17.05 |
| DoRA baseline | 95.61 / 68.21 | 95.61 / 66.23 | 95.44 / 47.52 | / 99.34‡ |
| DoRA +Cl | 94.95 / 29.04 | （Table 4 消融只做 LoRA，無 DoRA 分支） | 95.33 / 16.72 | （同左） |
| DoRA +Tr | 95.61 / 76.35 | （同上） | 95.77 / 49.50 | （同上） |
| DoRA +Cl+Tr | 95.28 / 41.36 | （同上） | 94.29 / 19.80 | （同上） |
| DoRA +Cl+Tr+Pt | 95.99 / 36.96 | （同上） | 94.95 / 14.30 | （同上） |

† 單次 seed=0 結果；11-seed 均值 26.05，最佳（seed=7）13.53——單一數字不代表典型表現，見上方 11-seed 章節。
‡ 論文只查到 ASR，未查到對應 CA 數字，非漏填。

**觀察：** InSent 的 Cl+Tr+Pt 全開，我們的 CA（95.83）與論文 CA（95.83）逐位元相同——
大概率為巧合（兩位小數的重合機率不算低），不視為驗證證據，僅記錄此現象。

**整體評估：** 方向性重現成立——baseline 幾乎完全記得 backdoor、Cl 單獨最有效、
完整機制疊加最佳，均與論文定性結論一致。數值重現未達成——所有欄位的 ASR 均高於論文對應值，
差距從 1.3 倍（LoRA+Cl+Tr+Pt, InSent: 10.67 vs 17.05，我們反而更低）到 8 倍以上不等
（LoRA+Cl, InSent: 16.50 vs 65.68，我們反而更低——注意這兩處我們的單次結果優於論文，
可能反映論文該格數字本身也是多次取最好、我們單次恰好幸運，而非我們方法更優）。
